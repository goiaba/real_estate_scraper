    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) \
        ...
        futures = {
            executor.submit(
                method_dispatcher.get(a_data.get("method", "none"),
                                      method_none), a_name, a_data): a_name
        ...
        for future in concurrent.futures.as_completed(futures):
            ...
            try:
                ...
                logger.exception(
                    "Retrieving house links from %s agency generated an"
                    " exception", a_name)
            else:
                ...
                if num_links_returned > 0:
                    ...
                    logger.info("%4i house links returned from %s agency",
                                num_links_returned, a_name)
                else:
                    logger.warning(
                        "%4i house links returned from %s agency."
                        " It will be ignored during this execution.",
                        num_links_returned, a_name)
...
if __name__ == "__main__":
    ...
    if total_house_links > 0:
        ...
        added_house_links = filter_added_house_links(current_house_links,
                                                     previous_house_links)
        removed_house_links = filter_removed_house_links(
            current_house_links, previous_house_links)
        logger.info("Total house links retrieved: %i", total_house_links)
        logger.info("Added house links")
        for l in added_house_links:
            logger.info(l)
        logger.info("Removed house links")
        for l in removed_house_links:
            logger.info(l)
        if (get_config("conf:email:enabled", False)
                and (added_house_links or removed_house_links)):
            send_email(total_house_links, added_house_links,
                       removed_house_links, previous_check_time,
                       current_check_time)
