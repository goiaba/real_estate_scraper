...
def get_config(key, default=None):
    def rec(curr, keys):
        if not keys: return curr
        if keys[0] not in curr:
            if default: return default
            raise Exception("\'%s\' nao pertence a estrutura \'%s\'." % (keys[0], curr))
        return rec(curr[keys[0]], keys[1:])
    if not key: raise Exception("Nenhuma chave de configuracao passada.")
    if key[0] == ':' or key[-1:] == ":": raise Exception("Chave invalida: %s" % key)
