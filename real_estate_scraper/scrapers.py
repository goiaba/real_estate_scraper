...
def method_none(a_name, a_data):
    logger.info("No method found to handle request to %s agency.", a_name)
    return []
...
def method_one(a_name, a_data):
    base_url = a_data.get("base_url")
    return method_one_four(a_name, a_data, lambda e: "%s%s" %
                           (base_url, e.get("url")))
...
def method_four(a_name, a_data):
    ...
    return method_one_four(
        a_name, a_data, lambda e: "%s/imoveis/%s" %
        (base_url, e.get("codigo")))
...
def method_two(a_name, a_data):
    ...
    base_url = a_data.get("base_url")
    prepend_base_url_in_search = a_data.get("prepend_base_url_in_search",
                                            False)
    result_search_string = tuple(
        map(str,
            a_data.get("result_search_string").split(",")))
    ...
    while True:
        ...
        for ref in refs:
            ...
            if (link.startswith(result_search_string)
